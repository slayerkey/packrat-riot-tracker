...
check("match.startedAt <= entry.createdAt + MANUAL_RECONCILE_WINDOW_MS" in service, "manual result reconciliation is bounded on both sides of its timestamp")
check("const nextCache = cache" in service and "void this.refresh(true)" in service, "manual session controls update local cache before asynchronous network refresh")
