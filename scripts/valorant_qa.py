...
for path in PROFILES:
    ...
    if label == "standard":
        ...
        standard_actions = {entry.get("action") for entry in keys if isinstance(entry, dict)}
        check("com.packrat.valorant-tracker.log-win" not in standard_actions and "com.packrat.valorant-tracker.log-loss" not in standard_actions, "standard profile keeps manual result controls out of the automatic tracking path")
        check("com.packrat.valorant-tracker.agent-win-rate" in standard_actions and "com.packrat.valorant-tracker.map-win-rate" in standard_actions, "standard profile uses the freed keys for useful automatic stats")
    if label == "xl":
...
model_source = (ROOT / "src" / "valorant" / "model.ts").read_text(encoding="utf-8")
...
check('event: "sendToPlugin"' in pi_js and 'type: "account-update"' in pi_js, "Property Inspector sends narrow account patches to the plugin")
check('event: "setGlobalSettings"' not in pi_js, "Property Inspector never writes the complete runtime global store")
check('event: "setSettings"' in pi_js, "Property Inspector keeps action-specific display controls in action settings")
check("updateAccountField" in service and "setGlobalSettings" in service, "plugin service is the sole global settings owner")
check("delete nextStore.session" in service and "delete nextStore.cache" in service, "changing tracked Riot identity clears prior player session and cache")
...
check("result.startedAt <= entry.createdAt + MANUAL_RECONCILE_WINDOW_MS" in session_source, "manual result reconciliation is bounded on both sides of its timestamp")
check("result.startedAt >= session.startedAt" in session_source and "startedAt < session.startedAt" in session_source, "session accounting never resurrects matches from before a reset")
check("AutomaticSessionResult" in model_source and "mergeAutomaticResults" in session_source and "session.automatic ?? []" in session_source, "automatic match ledger persists session results after Henrik recent history rolls forward")
...
check("session.automatic ?? []" in service, "manual fallbacks also baseline automatic matches that have rolled out of the cache")
check("appendManualResult" in service and "reconcileSession(nextSession" in service, "manual controls derive visible counts through the same session engine as API refreshes")
check("private mutationQueue" in service and "return this.enqueueMutation" in service, "session mutations are serialized so rapid taps cannot overwrite each other")
check("return await this.enqueueMutation" in service, "Henrik refresh commits are serialized with manual session writes")
check("private lastRevision" in service and "nextRevision <= this.lastRevision" in service, "echoed or stale global settings events cannot repaint older session state")
...
check("AbortSignal.timeout(API_TIMEOUT_MS)" in henrik and "API_TIMEOUT_MS = 12_000" in henrik, "Henrik API requests have a bounded timeout")
check("AbortSignal.timeout(ASSET_TIMEOUT_MS)" in henrik and "ASSET_TIMEOUT_MS = 6_000" in henrik, "remote artwork cannot indefinitely stall a refresh")
check("shouldUseRegionFallback" in henrik and "error.status >= 500" in henrik, "region discovery falls back on transient Henrik and network failures")
check("matchDrawn" in henrik and 'result = "draw"' in henrik, "competitive draws are recognized before false team-loss flags")
...
check("lastImageByAction" in actions_source and "lastImageByAction.get(instance) === image" in actions_source, "dynamic keys suppress duplicate setImage IPC")
check("invalidateKeyImage(ev.action)" in actions_source, "duplicate image cache is invalidated when a key becomes visible again")
check("Promise.all" in actions_source, "multi-key repaints are dispatched concurrently instead of serially")
check("session-reset" in actions_source and 'renderControl("RESET", "TAP TWICE"' in actions_source and 'renderControl("TAP AGAIN", "RESET SESSION"' in actions_source, "session reset requires an explicit two-tap confirmation")
